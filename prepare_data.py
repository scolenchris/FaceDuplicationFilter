...
def prepare_data(
    ...
    assert os.path.exists(
        embedding_location
    ), f" Given location for embeddings does not exist! : {embedding_location}"
    assert os.path.exists(
        label_location
    ), f" Given location for quality labels does not exist! : {label_location}"
    assert os.path.exists(
        save_location
    ), f" Given save location does not exist! : {save_location}"
    ...
    for path, quality in tqdm(
        quality_scores.items(), " Combining quality scores and image locations "
    ):
    ...
    quality_threshold = sorted(
        quality_scores.items(), key=lambda x: x[1], reverse=True
    )[int(beta * len(quality_scores.items()))][1]

    average_embeddings = {}
    # Calculate the mean high quality embedding for all training identities
    for identity, paths_qualities in tqdm(
        list(identity_quality_scores.items()), " Calculating mean embeddings "
    ):
        # Sort all files of given identity by quality scores
        sorted_paths_qualities = sorted(
            paths_qualities, key=lambda x: x[1], reverse=True
        )
        # Select images over the quality threshold
        chosen = [
            (path, quality)
            for path, quality in sorted_paths_qualities
        ...
        for item, _ in chosen:
            average_embedding = torch.cat(
                (average_embedding, torch.from_numpy(embeddings[item]).unsqueeze(0)),
                dim=0,
            )
    ...
    for i, (identity, average_embedding) in enumerate(
        tqdm(average_embeddings.items(), " Combining average embeddings ")
    ):
        mapping[identity] = i
        reverse_mapping[i] = identity
        average_embedding_batch = torch.cat(
            (average_embedding_batch, average_embedding.unsqueeze(0)), dim=0
        )

    # Make new training data list
    training_items = []
    for identity, paths_qualities in tqdm(
        identity_quality_scores.items(), " Creating training list "
    ):
    ...
    dump_pickle(
        os.path.join(save_location, f"average_embedding_batch-{save_name}.pkl"),
        average_embedding_batch,
    )
    dump_pickle(
        os.path.join(save_location, f"training_items-{save_name}.pkl"), training_items
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-ll", "--label_location", type=str, default="./training_data/vggface2-qs.pkl"
    )
    parser.add_argument(
        "-el",
        "--embedding_location",
        type=str,
        default="./training_data/cosface-embeddings.pkl",
    )
