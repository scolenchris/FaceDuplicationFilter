...
def main(root_folder, selected_num=2):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # root_folder = "./face/extracted_faces_20241114_125400"
    ...
    for folder_name in os.listdir(root_folder):
        ...
        if os.path.isdir(folder_path):
            known_faces_folder = folder_path
            print(f"Processing folder: {known_faces_folder}")
        else:
            continue
        ...
        with open(output_csv_path, mode="w", newline="", encoding="utf-8") as csvfile:
            ...
            for combo_indices in tqdm(
                combinations(range(len(images)), selected_num),
                total=len(list(combinations(range(len(images)), selected_num))),
                ...
                known_encoding = np.mean(
                    combo_encodings, axis=0
                )  # 图片组合（2）的平均编码
